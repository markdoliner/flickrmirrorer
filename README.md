...
The mirroring process is time-stamped. This allows Flickr Mirrorer to resume
the process from the previously reached moment in time. (It makes use of the
Flickr API upload date parameter.) This approach is particularly useful
when having a large Flickr photostream. The timestamp is stored in a local
file, which you can manually modify or delete, if needed.
